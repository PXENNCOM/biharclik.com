...
import { 
  ...
  BiIdCard, BiMap, BiCheckCircle, BiRightArrowAlt, 
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-white font-sans text-gray-900">
      ...
      <div className="lg:w-5/12 bg-gray-900 text-white flex flex-col relative overflow-hidden min-h-[300px] lg:min-h-screen lg:fixed lg:left-0 lg:top-0 lg:h-full z-10">
        <img src={HeroImage} alt="Biharçlık Kurye" className="absolute inset-0 w-full h-full object-cover z-0 opacity-60" />
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            {error && (
              <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}
                className="bg-red-50 border-l-4 border-red-500 text-red-700 px-5 py-4 rounded-r-xl text-sm font-bold flex items-center gap-3 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                <AccountTypeBtn active={accountType === 'individual'} onClick={() => setAccountType('individual')}
                  icon={<BiUser size={24} />} title="Bireysel" desc="Kişisel gönderiler" />
                <AccountTypeBtn active={accountType === 'corporate'} onClick={() => setAccountType('corporate')}
                  icon={<BiBuilding size={24} />} title="Kurumsal" desc="Şirket gönderileri" />
              </div>
            </div>

            {/* DİNAMİK ALANLAR */}
            <AnimatePresence mode="wait">
              <motion.div key={accountType} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-6">
              ...
              <div className="group">
                ...
                <div className="relative">
                  ...
                  <textarea name="billing_address" rows="3"
                    className="w-full bg-white text-gray-900 text-sm rounded-2xl border-2 border-gray-100 focus:border-yellow-400 block pl-12 p-4 transition-all outline-none font-semibold placeholder-gray-300 resize-none"
            ...
            <div className="space-y-4 pt-4 bg-gray-50/50 p-6 rounded-3xl border border-gray-100">
              <ApprovalLink checked={formData.kvkk_accepted} name="kvkk_accepted" onChange={handleChange}
                label="KVKK Aydınlatma Metni'ni" onClick={() => { setKvkkModalType('aydinlatma'); setShowKvkkModal(true); }} />
              <ApprovalLink checked={formData.terms_accepted} name="terms_accepted" onChange={handleChange}
                label="Kullanım ve Üyelik Koşulları'nı" onClick={() => { setKvkkModalType('kosullar'); setShowKvkkModal(true); }} />
            </div>
